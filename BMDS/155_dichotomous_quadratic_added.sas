...
%macro boundBMD(BMR, CL,DATAIN, DATAOUT, ESTIMATES);
   ...
   proc nlmixed data=&DATAIN maxiter=1000;
      ...
      _x = -log(1 - (&BMR./(1 - _gamma)));
      _bmd = (_x / _bdose)**(1/2);
      call symput("BMD", _bmd);

      model obs ~ binomial(n, p);

      predict p out=prediction_quadratic;
   run;
   ...
   data prediction_quadratic2;
	  set prediction_quadratic;
   ...
   proc summary data=prediction_quadratic2 nway;
	  var sq_residual;
	  output out=prediction_quadratic3 (drop=_type_ _freq_) sum()= n=dosegroups;
   run;

   data prediction_quadratic3;
	  set prediction_quadratic3;
	  file print;
   ...
   *set up the initial parameters for the new likelihood;
   ...
   data plot_model;
	  do dose1=&MINDOSE. to &MAXDOSE. by ((&MAXDOSE. - &MINDOSE.)/100);
		  if dose1 = 0 then resp1 = &GG.;
	     	  ...
	     	  resp1 = &GG. + (1 - &GG.)*(1 - exp(-(&BB. * (dose1**2))));
		  end;
		  output;
	  end;
   run;
   ...
   data plot_quadratic;
	  set prediction_quadratic2 plot_model plot_bmd plot_BMDL plot_BMR;
	  label dose="Dose"
   ...
   proc sgplot data=plot_quadratic noautolegend;
	  ...
	  title1 "Quantal Quadratic Model, with BMR of &BMR. Added Risk for the BMD and &CL. Lower Confidence Limit for the BMDL";
