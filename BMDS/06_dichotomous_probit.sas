...
%macro boundBMD(BMR, CL, DATAIN, DATAOUT, ESTIMATES);
   ...
   proc nlmixed data=&DATAIN maxiter=1000;
	  ...
	  _a = CDF('NORMAL', _alpha);

      _bmd = ( probit ( &BMR.*(1 - _a) + _a) - _alpha ) / _beta;
      call symput("BMD", _bmd);

      model obs ~ binomial(n, p);

      predict p out=prediction_probit;  
   run;
   ...
   data prediction_probit2;
	  set prediction_probit;
   ...
   proc summary data=prediction_probit2 nway;
	  var sq_residual;
	  output out=prediction_probit3 (drop=_type_ _freq_) sum()= n=dosegroups;
   run;

   data prediction_probit3;
	  set prediction_probit3;
	  file print;
   ...
   *set up the initial parameters for the new likelihood;
      ...
      *fit this new “constrained” likelihood;
      proc nlmixed data= &DATAIN. maxiter=1000;
         ...
         *solve for the BETA parameter as a function of the BMD;
		 _a = CDF('NORMAL', _alpha);
         _beta =  ( probit ( &BMR.*(1 - _a) + _a) - _alpha ) / &BMDL.;
   ...
   data plot_probit;
	  set prediction_probit2 plot_model plot_bmd plot_BMDL plot_BMR;
	  label dose="Dose"
   ...
   proc sgplot data=plot_probit noautolegend;
	  ...
	  title1 "Probit Model, with BMR of &BMR. Extra Risk for the BMD and &CL. Lower Confidence Limit for the BMDL";
