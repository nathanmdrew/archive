...
oldtrain1 <- readRDS(file="C:/Users/vom8/OneDrive - CDC/+My_Documents/MyLargeWorkspace Backup/ENM Categories/Framework Update 2019/04_random_forests_OUTPUT/train1.rds")
oldtest1 <- readRDS(file="C:/Users/vom8/OneDrive - CDC/+My_Documents/MyLargeWorkspace Backup/ENM Categories/Framework Update 2019/04_random_forests_OUTPUT/test1.rds")

#remove dupes - index 114:121
oldtrain1a <- oldtrain1 %>% filter(index<114 | index>121) #-6 obs
oldtest1a <- oldtest1 %>% filter(index<114 | index>121) #-2 obs

oldtrain2a <- oldtrain1a %>% select(-index, -cluster.Ward, -rand, -material_type, -Nanomat_Treatment)
oldtrain2b <- oldtrain1a %>% select(-index, -cluster.Complete, -rand, -material_type, -Nanomat_Treatment)
...
oldvalidation3a <- predict(object=old.rf.Complete3, newdata=oldtest1a)
oldvalidation3b <- predict(object=old.rf.Ward3, newdata=oldtest1a)

oldtest1a$pred.Complete3 <- oldvalidation3a
oldtest1a$pred.Ward3 <- oldvalidation3b

rf3.confusion <- table(oldtest1a$cluster.Complete, oldtest1a$pred.Complete3)
rf3.confusion
...
ward3.confusion <- table(oldtest1a$cluster.Ward, oldtest1a$pred.Ward3)
ward3.confusion
#accuracy <- abs(as.integer(oldtest1a$cluster.Ward) - as.integer(oldtest1a$pred.Ward3))/nrow(oldtest1a)
#init.accuracy <- 1-sum(accuracy)
