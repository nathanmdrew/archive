...
% 86 cases of NOISH data using DK.

clc
clear

load('86case_NOISH.mat')
data1=table2array(NOISH86casedosedata(:,1:3));% column 1 is dose, column 2 is response, column 3 is case number
...
for ii= 1: length(case_number)
    ...
    title(str);
    filename=strcat('Case_',num2str(case_number(ii)),'.pdf');
%     saveas(fig,filename);

    %% compute the BMDs for each case using BMR=0.1
        %% --- TO DO: Change BMR to background+4%
    BMR=yBar(1,1)+0.04;
    xIntpol=(0:0.001:1)';
