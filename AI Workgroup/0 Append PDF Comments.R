...
# WFSH Comments
subset_dat <- read.csv('C:/Users/vom8/OneDrive - CDC/DSI AI Workgroup/m1q-slgz-g1vn-20241001.csv', header = TRUE)

## file path of the directory to download all attachments to
dest_dir <- 'C:/Users/vom8/OneDrive - CDC/DSI AI Workgroup/pdf_comments/'
...
for(i in c(1:length(subset_dat$Attachment.Files)))
{
  if(nchar(subset_dat$Attachment.Files[i]) > 0)
    ...
    for(j in c(1:length(url_split[[1]])))
...
pdf_files <- dir_ls(dest_dir, regexp = "\\.pdf$")
...
write.csv(subset_dat, 
          file="C:/Users/vom8/OneDrive - CDC/DSI AI Workgroup/m1q-slgz-g1vn-20241001_withOCR.csv",
          row.names=FALSE)
