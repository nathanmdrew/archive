...
def _get_file_url(file_path: Path) -> str:
    ...
    """
    try:
        # Get path relative to BASE_OUTPUT_DIR
        rel_path = file_path.resolve().relative_to(config.BASE_OUTPUT_DIR)
...
def create_server(session_manager: SessionManager) -> ServerFunc:
    ...
    def server(input: Inputs, output: Outputs, session: Session) -> None:
        ...
        def topic_visualization() -> ShinyTag:
            ...
            try:
                ...
                return sh.ui.tags.iframe(
                    ...
                    height=config.UI.DIMENSIONS['plot_height'],
                    width="100%",
                    title=config.UI.COMPONENTS['sections']['results']['topic_viz']
                )
            except Exception as e:
                logger.error(f"Error rendering topic visualization: {str(e)}")
                return sh.ui.p("Error generating visualization")
        ...
        async def handle_load_data() -> None:
            ...
            - df_initial.csv: Initial loaded data
            - Logs file creation in session manager
            """
            try:
        ...
        @render.ui
        def topic_visualization() -> ShinyTag:
            """Render topic visualization directly as React component."""
            viz_data = state.topic_viz_data.get()
            if not viz_data:
                return sh.ui.p("No visualization data available")
                
            return sh.ui.div(
                className="topic-visualization",
                children=[
                    sh.ui.output_ui("topic_viz_content")
                ]
            )
