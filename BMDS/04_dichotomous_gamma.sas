...
%macro boundBMD(BMR, CL,DATAIN, DATAOUT, ESTIMATES);
   ...
   proc nlmixed data=&DATAIN;
      ...
      bounds _gamma >= 0, _gamma <= 1, _beta >= 0, _alpha >= 1, _alpha <= 18; /* alpha restriction added 11aug2017 --- must be positive at least*/

      p = _gamma + (1 - _gamma)*cdf('GAMMA', _beta*dose, _alpha, 1);
     
	  _bmd = QUANTILE('GAMMA', &BMR., _alpha) / _beta;
      call symput("BMD", _bmd);

      model obs ~ binomial(n, p);

      predict p out=prediction_gamma;
   run;
   ...
   data prediction_gamma2;
	  set prediction_gamma;
   ...
   proc summary data=prediction_gamma2 nway;
	  var sq_residual;
	  output out=prediction_gamma3 (drop=_type_ _freq_) sum()= n=dosegroups;
   run;

   data prediction_gamma3;
	  set prediction_gamma3;
	  file print;
   ...
   *set up the initial parameters for the new likelihood;
      ...
      *fit this new “constrained” likelihood;
      proc nlmixed data= &DATAIN. maxiter=1000;
         ...
         *solve for the BETA parameter _BETA as a function of the BMD;
         _beta = QUANTILE('GAMMA', &BMR., _alpha) / &BMDL.;
   ...
   data plot_model;
	  do dose1=&MINDOSE. to &MAXDOSE. by ((&MAXDOSE. - &MINDOSE.)/100);
	      resp1 = &GG. + (1 - &GG.)*cdf('GAMMA', &BB.*dose1, &AA., 1);
		  output;
	  end;
   run;
   ...
   data plot_BMD (keep = dose2 resp2 label);
	  ...
	  pi_bmd = &GG. + (1 - &GG.)*cdf('GAMMA', &BB.*&BMD., &AA., 1);
   ...
   data plot_gamma;
	  set prediction_gamma2 plot_model plot_bmd plot_BMDL plot_BMR;
	  label dose="Dose"
   ...
   proc sgplot data=plot_gamma noautolegend;
	  ...
	  title1 "Gamma Model, with BMR of &BMR. Extra Risk for the BMD and &CL. Lower Confidence Limit for the BMDL";
