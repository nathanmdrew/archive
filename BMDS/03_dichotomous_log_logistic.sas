...
%macro boundBMD(BMR, CL,DATAIN, DATAOUT, ESTIMATES);
   ...
   proc nlmixed data=&DATAIN maxiter=1000;
      ...
      bounds _bdose >= 1, _gamma >=0, _gamma <= 1; /*_alpha >=0, _alpha <= 18;*/ /* alpha restriction added 11aug2017 */
      ...
      _logbmd = (log(&BMR. / (1-&BMR.)) - _alpha) / _bdose; /*EXTRA RISK*/
	  _bmd = exp(_logbmd);
      call symput("BMD", _bmd);

      model obs ~ binomial(n, p);

      predict p out=prediction_log_logistic; 
   run;
   ...
   data prediction_log_logistic2;
	  set prediction_log_logistic;
   ...
   proc summary data=prediction_log_logistic2 nway;
	  var sq_residual;
	  output out=prediction_log_logistic3 (drop=_type_ _freq_) sum()= n=dosegroups;
   run;

   data prediction_log_logistic3;
	  set prediction_log_logistic3;
	  file print;
   ...
   *set up the initial parameters for the new likelihood;
      ...
      %LET BOUNDS = _bdose >= 1, _gamma >=0, _gamma <= 1;  
      /*%LET SMODEL = ;*/
      ...
      *fit this new “constrained” likelihood;
      proc nlmixed data= &DATAIN. maxiter=1000;
         ...
         *solve for the ALPHA parameter _ALPHA as a function of the BMD;
         _alpha = log(&BMR. / (1-&BMR.)) - log(&bmdl)*_bdose ;
   ...
   data plot_model;
	  do dose1=&MINDOSE. to &MAXDOSE. by ((&MAXDOSE. - &MINDOSE.)/100);
		  if dose1 = 0 then resp1 = &GG.;
	     	  ...
	     	  resp1 = &GG. + (1 - &GG.) / (1 + exp(-&AA. - &BB.*log(dose1)));
		  end;
		  output;
	  end;
   run;
   ...
   data plot_BMD (keep = dose2 resp2 label);
	  ...
	  pi_bmd = &GG. + (1 - &GG.) / (1 + exp(-&AA. - &BB.*log(&BMD.)));
   ...
   data plot_log_logistic;
	  set prediction_log_logistic2 plot_model plot_bmd plot_BMDL plot_BMR;
	  label dose="Dose"
   ...
   proc sgplot data=plot_log_logistic noautolegend;
	  ...
	  title1 "Log Logistic Model, with BMR of &BMR. Extra Risk for the BMD and &CL. Lower Confidence Limit for the BMDL";
