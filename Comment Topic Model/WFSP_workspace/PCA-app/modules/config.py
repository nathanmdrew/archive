...
# Base directories - working up from code directory to project root
CODE_DIR: Final[Path] = Path(__file__).parent.parent.resolve()
PROJECT_DIR: Final[Path] = CODE_DIR.parent  # Up one level from code dir
BASE_OUTPUT_DIR: Final[Path] = PROJECT_DIR / "outputs"

# Create outputs directory if it doesn't exist
BASE_OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Other directories relative to outputs
TEMP_DIR: Final[Path] = BASE_OUTPUT_DIR / "temp"
CACHE_DIR: Final[Path] = BASE_OUTPUT_DIR / "cache"
DATA_DIR: Final[Path] = PROJECT_DIR / "data"  # Sibling to code dir

# Ensure other directories exist
for directory in [TEMP_DIR, CACHE_DIR, DATA_DIR]:
...
TOPIC_MODELING: Final[Dict[str, Any]] = {
    ...
    'MIN_TOPIC_SIZE': 4,  # Added for UI compatibility
    
    # Seed topics for guided modeling - using DEFAULT key for backwards compatibility
    'SEED_TOPICS': {
        'DEFAULT': [
