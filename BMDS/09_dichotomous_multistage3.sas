...
%macro boundBMD(BMR, CL, DATAIN, DATAOUT, ESTIMATES);
   ...
   proc nlmixed data=&DATAIN. maxiter=1000;
	  ...
	  call symput("BETA3", _bdose3);

      model obs ~ binomial(n, p);

      predict p out=prediction_ms3;
   run;
   ...
   proc summary data=prediction_ms3_2 nway;
	  var sq_residual;
	  output out=prediction_ms3_3 (drop=_type_ _freq_) sum()= n=dosegroups;
   run;

   data prediction_ms3_3;
	  set prediction_ms3_3;
	  file print;
   ...
   /*   Solve for BMD (root)  */
   /*   Beta1*BMD + BETA2*BMD^2 + BETA3*BMD^3 + log(1-BMR) = 0   */
   proc iml;
      start Func(x);
	     BMD = x[1]; /* one dimensional vector */
	     f = &BETA1.*BMD + &BETA2.*BMD**2 + &BETA3.*BMD**3 + log(1-&BMR.);
		 return(f);
	  finish Func;

	  start Deriv(x);
	     BMD = x[1];
		 dfdx = &BETA1. + 2*&BETA2.*BMD + 3*&BETA3.*BMD**2;
		 return(dfdx);
	  finish Deriv;
   ...
   *set up the initial parameters for the new likelihood;
      ...
      %LET BOUNDS = _gamma >= 0, _gamma <= 1, _bdose2 >= 0, _bdose3 >= 0;
      /*%LET SMODEL = ;*/
      ...
      *fit this new “constrained” likelihood;
      proc nlmixed data= &DATAIN maxiter=1000;
         ...
         _BDOSE1 = (log(1-&BMR.) + _bdose2*&BMDL.**2 + _bdose3*&BMDL.**3)/(-&BMDL.);
   ...
   data _null_;
	  ...
	  if parameter="_bdose3" then do;
		  call symput ("BB3", estimate);
	  end;
   run;
   ...
   data plot_model;
	  do dose1=&MINDOSE. to &MAXDOSE. by ((&MAXDOSE. - &MINDOSE.)/100);
		  if dose1 = 0 then resp1 = &GG.;
	     	  ...
	     	  resp1 = &GG. + (1 - &GG.)*(1 - exp(-((&BB1. * (dose1) + (&BB2. * dose1**2) + (&BB3. * dose1**3)))));
		  end;
		  output;
	  end;
   run;
   ...
   data plot_BMD (keep = dose2 resp2 label);
	  ...
	  pi_bmd = &GG. + (1 - &GG.)*(1 - exp(-((&BB1. * (&BMD.) + (&BB2. * &BMD.**2) + (&BB3. * &BMD.**3)))));
   ...
   data plot_ms3;
	  set prediction_ms3_2 plot_model plot_bmd plot_BMDL plot_BMR;
	  label dose="Dose"
   ...
   proc sgplot data=plot_ms3 noautolegend;
	  ...
	  title1 "Multistage (Degree 3) Model, with BMR of &BMR. Extra Risk for the BMD and &CL. Lower Confidence Limit for the BMDL";
