...
* read in the dataset ;
proc import datafile="Z:\MyLargeWorkspace Backup\ENM Categories\28 - 60 Days Post Exposure\NIOSHdosedata_postexp_28_60_v2.csv" out=d1 dbms=csv replace;
run;
...
/* adjust for missing Sager cases 8, 9 */
data d5;
	set d4;
	by casenumber;
	jj=casenumber;
	if casenumber >= 10 then jj=jj-2;
run;
...
%macro trend();
	...
	%do ii=1 %to 20;
		...
		/* Raw Data */
		ods graphics on;
		ods pdf file="Z:\MyLargeWorkspace Backup\ENM Categories\28 - 60 Days Post Exposure\Trend Diagnostics\Study_Case_&kk._Diagnostics.pdf";
		proc glm data=d5 (where=(jj=&ii.)) plots=diagnostics;
			class dep_dose_amount2;
			model samppmnper = dep_dose_amount2;
			means dep_dose_amount2 / hovtest;
			ODS output HOVFTest=HOV_Case_&kk. OverallANOVA=ANOVA_Case_&kk.;
		quit;
		ods pdf close;
		ods graphics off;
		...
		/* %if &pval. < 0.05 %then %do; */
			...
			ods graphics on;
			ods pdf file="Z:\MyLargeWorkspace Backup\ENM Categories\28 - 60 Days Post Exposure\Trend Diagnostics\Study_Case_&kk._T_Diagnostics.pdf";
			proc glm data=d5 (where=(jj=&ii.)) plots=diagnostics;
				class dep_dose_amount2;
				model log10_samppmnper = dep_dose_amount2;
				means dep_dose_amount2 /  hovtest;
				ODS output HOVFTest=HOV_T_Case_&kk. OverallANOVA=ANOVA_T_Case_&kk.;
			quit;
			ods pdf close;
			ods graphics off;
		...
		data trend_&ii. (drop=pvalcheck);
			casenumber=&ii.;
			if casenumber >= 8 then casenumber=casenumber+2;
...
proc sort data=d1 out=d6(keep=casenumber study_key studyref material material_type pe_d scale) nodupkey;
...
/*  of the 7 cases with no trend, 6 also visually appear to lack a trend  --- Case 12 likely has a trend, strong F test, cant get constant var */

data d7;
	set d7;
	if casenumber=12 then trend="Y";
run;

proc export data=d7 outfile="Z:\MyLargeWorkspace Backup\ENM Categories\28 - 60 Days Post Exposure\Trend Tests.xlsx" dbms=excel replace;
run;



/* Case 6 had a Levene's p-value of <0.0001, so it didn't copy over */
/* Manually checked trend outputs, viz below --- no trend */
proc sgplot data=d1 (where=(casenumber=6));
	title "Visual Trend Check for Case Number 6";
	scatter x=dep_dose_amount2 y=samppmnper;
run;
