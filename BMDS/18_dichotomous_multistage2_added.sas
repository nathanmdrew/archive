...
%macro boundBMD(BMR, CL,DATAIN, DATAOUT, ESTIMATES);
   ...
   proc iml;
      start Func(x);
	     ...
	     f = &BETA1.*BMD + &BETA2.*BMD**2 + log(1-(&BMR. / (1-&GAMMA.)));
		 return(f);
	  finish Func;
   ...
   *set up the initial parameters for the new likelihood;
      ...
      *fit this new “constrained” likelihood;
      proc nlmixed data= &DATAIN maxiter=1000;
         ...
         _BDOSE1 = (log(1-(&BMR. / (1-_gamma))) + _bdose2*&BMDL.**2)/(-&BMDL.);
   ...
   proc sgplot data=plot_ms2 noautolegend;
	  ...
	  title1 "Multistage (Degree 2) Model, with BMR of &BMR. Added Risk for the BMD and &CL. Lower Confidence Limit for the BMDL";
