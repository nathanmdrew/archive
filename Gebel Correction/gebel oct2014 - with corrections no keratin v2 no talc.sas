/*///////////////////////////////////////////////////
///  analyze eileen's version of gebel's data
...
///
///   no keratin cysts considered
///   only full term data considered
/*/

/*
	v2:  change MEAN calls in Summaries to SUM
		 Taking an average of a weighted average was penalizing Micro, inflating the ratio

         Using the final dataset per Randy

         Correct Cumulative Lung Burden metrics
			remove unneeded standardization
			lung burden is reported as mg/g in the database
*/


options nocenter nonumber nodate ls=80 formdlim="*" mprint symbolgen;

%let dir = \\cdc.gov\private\L606\vom8\Gebel Correction Project 2014-09\Data\ ;
libname c "&dir.";
...
data WORK.GEBEL_DATA    ;
	...
	infile "&dir.database_nokeratinv2.csv" delimiter = ',' MISSOVER DSD lrecl=32767 firstobs=2 ;
	informat Study $20. ;
	informat rat $8. ;
	informat par_type $5. ;
...
data pot;
	set pot;
	if study="Talc_NTP93" then delete;
run;
...
data save.pot_nokeratin_v2_notalc;
	set pot;
run;
