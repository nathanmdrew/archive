...
datafile = 'wfs_rfi_original comments.csv'

current_date = datetime.now().strftime('%Y-%m-%d')
output_path = f'/Volumes/edav_dev_ddnid_niosh/wfsp/datafiles/output/{current_date}/'
...
doc_all["DocumentID_TrackingNumber"] = doc_all["Document_ID"] + "_" + doc_all["Tracking_Number"]

#doc_all.head(4)  # Display the first row of the DataFrame
...
df = doc_all[["DocumentID_TrackingNumber", "Comment_with_Attachment"]]  # Select only the Document_ID and Comment columns from the DataFrame

df = df.rename(columns={"Comment_with_Attachment": "Comment"}) #renaming to just "Comment" to not have to change it everywhere in the script
...
duplicated_ids = duplic_comments["DocumentID_TrackingNumber"].to_list()  # Convert Document_ID column to a list of duplicated IDs
...
df['Is_Duplicated'] = df['DocumentID_TrackingNumber'].apply(lambda x: check_duplicate(x))  # Marks first duplicate as true in a list of unique docs
df.head(2)
...
ids = df.DocumentID_TrackingNumber.values.tolist()  # Convert 'Document_ID' column to list
...
umap_model = UMAP(n_neighbors=3, n_components=2, min_dist=0.75, metric='euclidean', random_state=42)

# Step 3 - Cluster reduced embeddings
hdbscan_model = HDBSCAN(min_cluster_size=3, metric='euclidean', cluster_selection_method='leaf', prediction_data=True, min_samples=3) 
...
for n in range(3,12):  # Range of 3,32 gives 2 to 30 topic solutions plus a cluster with outliers
    topic_model = BERTopic(nr_topics=n,
                        ...
                        top_n_words=10,                           # Shows top N words in the outputs
                        min_topic_size=3
    ...
    topic_model.update_topics(docs, topics=new_topics, vectorizer_model=vectorizer_model, ctfidf_model=ctfidf_model, top_n_words = 10)  # Update topics with new topics and models
    d = {
...
coherence_scores_df = pd.DataFrame(coherence_scores, columns = ["Coherence Scores"])
max_coherence = coherence_scores_df["Coherence Scores"].idxmax()
number_topics = max_coherence+3
...
interest_model_info = get_model(all_models, number_topics)  # Select model of interest with N topics

# Save the bar chart visualization
fig_barchart = interest_model_info["model"].visualize_barchart(
    n_words=5, width=250, height=1200, top_n_topics=10
)
...
interest_model_info = get_model(all_models, 9)  # Select model of interest with a N serving as a topic solution
# Note that I need to add +1 compared to plot values as there is an empty cluster with outliers

# COMMAND ----------

interest_model_info["model"].visualize_barchart(n_words=5, width = 250, height = 1200, top_n_topics=10)  # Visualize the top 45 topics with a bar chart, showing 40 words per topic, with specified width and height
...
interest_model_info["model"].visualize_topics()
...
interest_model_info = get_model(all_models, 9)  # Example with 16 topics
...
rel_cols = all_docs_allCols[["Document", "Topic", "Probability", "Name"]]  # Select relevant columns
rel_cols["ID"] = df["DocumentID_TrackingNumber"].values  # Add Document_ID column
...
#sorted_df = sorted_df.drop(['DocumentID_TrackingNumber'], axis=1)  # Optional
df_merged = pd.merge(doc_all, sorted_df, how='left', left_on=['DocumentID_TrackingNumber'], right_on=['ID'])  # Merge doc_all with sorted_df on Document_ID and ID
...
split_col = split(final_df['Name'], '_')

# Select the first 3 words and concatenate them back into a single string
final_df = final_df.withColumn('first_3_words', concat_ws('_', split_col[1], split_col[2], split_col[3]))
...
pandas_df.to_csv(f'{output_path}/{datafile}_comments_rev1.csv', mode='w', index=False)
print(pandas_df)
...
# writing output of BERTopic models to CSV to see what it looks like
all_docs_allCols.to_csv(f'{output_path}/BERTopic_model_output.csv', mode='w', index=False)